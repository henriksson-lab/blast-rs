...
//! match sequence) are tracked by [`BlastForbiddenRanges`]. The dispatch functions
...
/// Rust canonical spelling for the C BLbasic Smith-Waterman
/// score-only primitive; the old provenance audit mechanically expanded
/// `BLbasic` as `b_lbasic`.
...
pub const COMPO_SCORE_MIN: i32 = i32::MIN / 2;
    ...
    for query_pos in 0..query_length {
        ...
        for match_seq_pos in 0..match_seq_length {
            ...
            new_score = prev_score_no_gap_match_seq
                + pssm[query_offset + query_pos][match_seq[match_seq_pos] as usize];
            if new_score < 0 {
                new_score = 0;
            }
            if new_score < prev_score_gap_match_seq {
...
pub struct BlastForbiddenRanges {
    /// `capacity` — length of the query sequence covered by the
    /// `num_forbidden` and `ranges` arrays.
    pub capacity: i32,
...
pub fn blast_forbidden_ranges_initialize(capacity: i32) -> BlastForbiddenRanges {
    ...
    BlastForbiddenRanges {
        ...
        ranges: vec![vec![0i32; 2]; cap],
        is_empty: true,
    }
}

/// NCBI: Blast_ForbiddenRangesRelease (smith_waterman.c:461).
pub fn blast_forbidden_ranges_release(forbidden: &mut BlastForbiddenRanges) {
    forbidden.ranges.clear();
    forbidden.num_forbidden.clear();
    forbidden.capacity = 0;
...
pub fn blast_forbidden_ranges_push(
    ...
    if query_start < 0 || query_end < query_start || query_end > forbidden.capacity {
        return -1;
    }
    for f in query_start as usize..query_end as usize {
        let last = 2 * forbidden.num_forbidden[f] as usize;
        if last != 0 {
            if forbidden.ranges[f].try_reserve_exact(2).is_err() {
                return -1;
            }
            forbidden.ranges[f].push(match_start);
            forbidden.ranges[f].push(match_end);
        } else {
            forbidden.ranges[f][0] = match_start;
            forbidden.ranges[f][1] = match_end;
        }
...
/// Rust canonical spelling for the C BLspecial Smith-Waterman
/// score-only primitive; the old provenance audit mechanically expanded
/// `BLspecial` as `b_lspecial`.
///
...
/// Rust canonical spelling for the C BLspecial Smith-Waterman
/// find-start primitive; the old provenance audit mechanically expanded
/// `BLspecial` as `b_lspecial`.
///
...
mod tests {
    ...
    fn test_forbidden_ranges_basic() {
        ...
        let mut fr = blast_forbidden_ranges_initialize(q.len() as i32);
        assert!(fr.is_empty);
        let (s1, m_end1, q_end1) = blast_smith_waterman_score_only(&s, &q, &m, 11, 1, Some(&fr));
        assert_eq!(s1, 38);
        let (s1_back, m_start1, q_start1) =
            blast_smith_waterman_find_start(&s, &q, &m, 11, 1, m_end1, q_end1, s1, Some(&fr));
        assert_eq!(s1_back, 38);
        assert_eq!(q_start1, 0);

        // Mark the first occurrence as forbidden, do the second pass.
        blast_forbidden_ranges_push(
            &mut fr,
            q_start1 as i32,
            q_end1 as i32 + 1,
            m_start1 as i32,
        ...
        assert!(!fr.is_empty);
        let (s2, m_end2, _q_end2) = blast_smith_waterman_score_only(&s, &q, &m, 11, 1, Some(&fr));
    ...
    fn test_forbidden_ranges_clear_resets_is_empty() {
        let mut fr = blast_forbidden_ranges_initialize(10);
        blast_forbidden_ranges_push(&mut fr, 0, 5, 0, 7);
        assert!(!fr.is_empty);
        blast_forbidden_ranges_clear(&mut fr);
        assert!(fr.is_empty);
    ...
    fn test_smith_waterman_pssm_respects_query_offset() {
        ...
        let (score, m_end, q_end) =
            blast_smith_waterman_score_only_pssm(&s, &q, &pssm, 3, 11, 1, None);
        assert_eq!(score, 20);
        assert_eq!(m_end, 3);
        assert_eq!(q_end, 3);

        let (score2, m_start, q_start) = blast_smith_waterman_find_start_pssm(
            &s, &q, &pssm, 3, 11, 1, m_end, q_end, score, None,
        );
        assert_eq!(score2, 20);
        assert_eq!(m_start, 0);
        assert_eq!(q_start, 0);
    ...
    fn test_smith_waterman_pssm_forbidden_ranges() {
        ...
        let (score1, m_end1, q_end1) =
            blast_smith_waterman_score_only_pssm(&s, &q, &pssm, 2, 11, 1, Some(&fr));
        assert_eq!(score1, 20);
        let (score1_back, m_start1, q_start1) = blast_smith_waterman_find_start_pssm(
            &s,
            &q,
            &pssm,
            2,
        ...
        let mut all_forbidden = blast_forbidden_ranges_initialize(q.len() as i32);
        blast_forbidden_ranges_push(
            &mut all_forbidden,
            0,
            q.len() as i32,
        ...
        let (score3, _, _) =
            blast_smith_waterman_score_only_pssm(&s, &q, &pssm, 2, 11, 1, Some(&all_forbidden));
        assert_eq!(score3, 0);
    }
}
