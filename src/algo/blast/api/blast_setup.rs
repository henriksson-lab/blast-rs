...
mod tests {
    ...
    fn gap_align_set_up_updates_gumbel_db_length_and_allocates_outputs() {
        let src = setup_seq_src(vec![vec![b'A'; 300], vec![b'C'; 150]], 900, 2);
    ...
    fn main_setup_runs_filter_lookup_mask_and_score_init() {
        ...
        assert_eq!(
            lookup_segments.as_ref().map(|node| *node.ssr),
            Some(SSeqRange { left: 0, right: 19 })
        );
        assert_eq!(mask.as_ref().map(|mask| mask.seqloc_array.len()), Some(1));
        assert!(mask
            .as_ref()
            .is_some_and(|mask| mask.seqloc_array[0].is_none()));
    ...
    fn subject_totals_match_c_fallback_order() {
        let src = setup_seq_src(vec![b"AAAA".to_vec(), b"CC".to_vec()], 100, 5);
        let mut total = 0;
        let mut num = 0;
        blast_get_subject_totals(Some(&src), Some(&mut total), Some(&mut num));
        assert_eq!((total, num), (100, 5));

        let src = setup_seq_src(vec![b"AAAA".to_vec(), b"CC".to_vec()], 0, 0);
        blast_get_subject_totals(Some(&src), Some(&mut total), Some(&mut num));
        assert_eq!((total, num), (6, 2));

        let empty = setup_seq_src(Vec::new(), 0, 0);
        ...
        assert_eq!(
            blast_seq_src_get_sequence(&src, &mut arg),
            BLAST_SEQSRC_SUCCESS
        );
        assert_eq!(arg.seq.as_ref().map(|seq| seq.length), Some(4));
        blast_seq_src_release_sequence(&src, &mut arg);
        assert!(arg.seq.is_none());
    }

    #[test]
    fn calc_eff_lengths_updates_search_space() {
        let mut qi = QueryInfo {
            num_queries: 1,
            contexts: vec![ContextInfo {
                query_offset: 0,
    ...
    fn one_subject_update_parameters_recomputes_dependent_cutoffs() {
        ...
        let mut hit = HitSavingParameters {
            ...
            link_hsp_params: Some(LinkHSPParameters {
                gap_prob: 0.5,
                gap_size: 40,
                overlap_size: 9,
                gap_decay_rate: 0.5,
