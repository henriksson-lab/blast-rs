...
fn has_flag(p: ProgramType, mask: u32) -> bool { (p & mask) != 0 }
...
pub fn is_valid(p: ProgramType) -> bool {
    matches!(p, BLASTP | BLASTN | BLASTX | TBLASTN | TBLASTX | PSI_BLAST | RPS_BLAST | MAPPING)
}

pub fn num_contexts(p: ProgramType) -> u32 {
    if is_nucleotide(p) || is_mapping(p) { 2 }
    else if query_is_translated(p) { 6 }
    else { 1 }
}
