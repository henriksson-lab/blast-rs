...
fn sanitize_sam_field(value: &str) -> String {
    value
        .chars()
        .map(|c| {
            if c == '\t' || c == '\n' || c == '\r' {
...
fn build_subject_as_read_cigar(
    query_aln: Option<&[u8]>,
...
fn sam_gap_count(query_aln: Option<&[u8]>, subject_aln: Option<&[u8]>) -> i32 {
